...
private:
	...
	void get_obstacle_vector(double *minrange, sensor_msgs::LaserScan scan_all, int *obs_count, double *obs_vector_x, double *obs_vector_y,bool lidar_or_not, double *obs_dir_r, double *obs_dir_theta);
        void calculate_obstacle_vector(double *obs_r,double *obs_theta, double *minrange, int *obs_count, double *obs_vector_x, double *obs_vector_y);
	...
	static const double MaxRobotAngularVelocity;
  	...
  	bool switch_avoid;
	//============Module============//	
	bool camera_detect_obs_or_not;
	bool lidar_detect_obs_or_not;
	bool avoid_obs_or_not;
        double obs_dir_r[3];
        double obs_dir_theta[3];
	bool both;
	int both_count;
	double t1, t2;
	std_msgs::Float64 during;
};
//=========status============//
...
NavigationController::NavigationController(tf::TransformListener& tf) : tf_(tf)
{
	...
	scan_sub_ = nh_.subscribe("scan_filtered", 1, &NavigationController::get_scan, this);
	...
	during_time = nh_.advertise<std_msgs::Float64>("during_time", 1);
	...
	if(switch_avoid == false)
		kAvoidRadius_ = 0.5;
	//=======Module======//
	if(!para_node.getParam("camera_detect_obs_or_not", camera_detect_obs_or_not))
    		camera_detect_obs_or_not = false;
	if(!para_node.getParam("lidar_detect_obs_or_not", lidar_detect_obs_or_not))
    		lidar_detect_obs_or_not = false;
	if(!para_node.getParam("avoid_obs_or_not", avoid_obs_or_not))
    		avoid_obs_or_not = false;
	both = false;
	both_count = 0;
...
void NavigationController::run()
{
	...
	while (ros::ok()) {
		...
		if(lidar_detect_obs_or_not == true && camera_detect_obs_or_not == true)
			both = true;
		else
			both = false;
		both_count = 0;
		if(lidar_detect_obs_or_not == true)
			get_obstacle_vector(&minrange, scan_, &obs_count, &obs_vector_x, &obs_vector_y,true,obs_dir_r,obs_dir_theta);
		if(camera_detect_obs_or_not == true)
			get_obstacle_vector(&minrange, scan_camera, &obs_count, &obs_vector_x, &obs_vector_y,false,obs_dir_r,obs_dir_theta);
		if(camera_detect_obs_or_not == true || lidar_detect_obs_or_not == true)
			calculate_obstacle_vector(&obs_r, &obs_theta,&minrange, &obs_count, &obs_vector_x, &obs_vector_y);
		...
		during.data = (t2-t1);
		ROS_INFO_STREAM("Time : "<<during.data);
		during_time.publish(during);
		//====================================//
		last_cmd_ = cmd;
		ros::spinOnce();
		rate.sleep();
...
{
	scan_ = *scan;
        //ROS_INFO_STREAM("scan_angle_min = " << scan_.angle_min);
	//ROS_INFO_STREAM("scan_angle_max = " << scan_.angle_max);
}
void NavigationController::get_scan_camera(const sensor_msgs::LaserScan::ConstPtr& scan__camera)
{
	scan_camera = *scan__camera;
        //ROS_INFO_STREAM("scan_camera_angle_min = " << scan_camera.angle_min);
	//ROS_INFO_STREAM("scan_camera_angle_max = " << scan_camera.angle_max);
}
void NavigationController::get_obstacle_vector(double *minrange, sensor_msgs::LaserScan scan_all, int *obs_count, double *obs_vector_x, double *obs_vector_y,bool lidar_or_not, double *obs_dir_r, double *obs_dir_theta)
{
	int beam_range = scan_all.ranges.size() / beam_num;
	double beam_minrange[beam_num];		//m
	double beam_minrange_2[beam_num];	//m
	double beam_true_d[beam_num];
	double beam_angle[beam_num];		//rad
	double adjust;				//m
	double adjust_2;			//m

	*minrange = scan_all.range_max;		//m
	for(int i = 0; i < beam_num; i++) {
		if(i >= (beam_num / 5) && i <= (beam_num * 4 / 5)){	// beam_num / 5 && beam_num* 4 / 5
				adjust = 0.35;	//0.8
				adjust_2 = 0.0;
		}
		else{
		...
		beam_minrange[i] = scan_all.range_max;
		beam_minrange_2[i] = scan_all.range_max;
		for(int j = i * beam_range; j < (i+1) * beam_range; j++) {
			if(std::isnormal(scan_all.ranges[j])){
				beam_minrange[i] = std::min(beam_minrange[i],(double)std::max((scan_all.ranges[j]-adjust), 0.0));
				beam_true_d[i] = (double)scan_all.ranges[j];
				beam_minrange_2[i] = std::min(beam_minrange_2[i],(double)std::max((scan_all.ranges[j]-adjust_2), 0.0));
			}
		}
		...
		if(lidar_or_not == true)
			beam_angle[i] = -(scan_all.angle_min + ((double)i + 0.5) * (double)beam_range * scan_all.angle_increment);
		else	
			beam_angle[i] = scan_all.angle_min + ((double)i + 0.5) * (double)beam_range * scan_all.angle_increment;	
		if(lidar_or_not == true){
			if (beam_angle[i] > 0){
				beam_angle[i] = 3.14 - beam_angle[i]; //1.7
				if(beam_angle[i] > 2.5)
					beam_minrange[i] = scan_all.range_max;			
			}
			else if (beam_angle[i] < 0){
				beam_angle[i] = -3.14 - beam_angle[i]; //1.7
				if(beam_angle[i] < -2.3)
					beam_minrange[i] = scan_all.range_max;	
			}
			if((i == 6) || (i == 8) || (i == 24) || (i == 25))
				beam_minrange[i] = scan_all.range_max;	
		}
		//else
			//beam_angle[i] = - beam_angle[i];		
		ROS_INFO_STREAM("angle " << i << ": " << beam_angle[i]  << " min : " << beam_true_d[i]);
	...
	for (int i = 0; i < beam_num; i++) {
		...
		if (beam_minrange[i] < kAvoidRadius_ && beam_minrange[i] > 0) {
			...
			*obs_count = *obs_count + 1;
			//ROS_INFO_STREAM("obs_count " << *obs_count);

		}
	}
	/*
	if(lidar_or_not == true)
		ROS_INFO_STREAM("Lidar : obs_count " << *obs_count<< "obs_vector_x " << *obs_vector_x << " obs_vector_y " << *obs_vector_y );
	else
		ROS_INFO_STREAM("Camera : obs_count " << *obs_count<< "obs_vector_x " << *obs_vector_x << " obs_vector_y " << *obs_vector_y );
	*/
	//=====================obs topic========================//
	//-------------------------------------------------
        //=======================//
        //double obs_left[2] = {10.0, 0.0};
        //double obs_min[2] = {10.0, 0.0};
        //double obs_right[2] = {10.0, 0.0};
        std_msgs::Float64MultiArray obs_array;
        obs_array.data.clear();
	...
	for (int i = 0; i < beam_num; i++) {
		if (beam_minrange[i] < kAvoidRadius_ && beam_minrange[i] > 0) {
			double range = beam_true_d[i];
                        if(beam_angle[i]  <= 2.6 && beam_angle[i]   > 0.67){
                           //obs_left[0] += temp * cos(beam_angle[i]);
			   //obs_left[1] += temp * sin(beam_angle[i]);
			   //obs_dir_count[0]++;
			   obs_dir_count[0]++;
			   if(range < obs_dir_r[0]){
			   	obs_dir_r[0] = range;
				obs_dir_theta[0] = beam_angle[i];
			   }
                        }
                        else if(beam_angle[i]   >= -0.67 && beam_angle[i]   <= 0.67){
                           //obs_min[0] += temp * cos(beam_angle[i]);
			   //obs_min[1] += temp * sin(beam_angle[i]);
			   //obs_dir_count[1]++;
			   obs_dir_count[1]++;
			   if(range < obs_dir_r[1]){
			   	obs_dir_r[1] = range;
				obs_dir_theta[1] = beam_angle[i];
			   }
                        }
                        else if(beam_angle[i]   >= -2.3 && beam_angle[i]   < -0.67){
                           //obs_right[0] += temp * cos(beam_angle[i]);
			   //obs_right[1] += temp * sin(beam_angle[i]);
			   //obs_dir_count[2]++;
			   obs_dir_count[2]++;
			   if(range < obs_dir_r[2]){
			   	obs_dir_r[2] = range;
				obs_dir_theta[2] = beam_angle[i];
			   }
                        }
		}
	}
	...
	ROS_INFO_STREAM("obs_left_count " << obs_dir_count[0] << " obs_left_r " << obs_dir_r[0] << " obs_left_theta " << obs_dir_theta[0]);
        ROS_INFO_STREAM("obs_min_count " << obs_dir_count[1] << " obs_min_r " << obs_dir_r[1] << " obs_min_theta " << obs_dir_theta[1]);
        ROS_INFO_STREAM("obs_right_count " << obs_dir_count[2] << " obs_right_r " << obs_dir_r[2] << " obs_right_theta " << obs_dir_theta[2]);
...
void NavigationController::calculate_obstacle_vector(double *obs_r,double *obs_theta, double *minrange, int *obs_count, double *obs_vector_x, double *obs_vector_y){
	std_msgs::Float64MultiArray obs_array;
	//obs_array.data.clear();
	//double apple;
	//*obs_count = 1;
	...
	if(*obs_count > 0) {
		...
		*obs_r = sqrt(pow(*obs_vector_x, 2) + pow(*obs_vector_y, 2));
		//apple = sqrt(pow(*obs_vector_x, 2) + pow(*obs_vector_y, 2));
...
void NavigationController::avoid_radiate(double *v_a, double *w_a,double user_velocity,double user_rotation,double obs_r,double obs_theta,double minrange)
{
	...
	double Ea = 1;
	Ea = pow(std::min(minrange / kAvoidRadius_, 1.0), 0.7); //0.23

	ROS_INFO_STREAM("Ea = " << Ea);

	//======Module=======//
	if(avoid_obs_or_not == true)
		Ea = 1;
	//===================//
