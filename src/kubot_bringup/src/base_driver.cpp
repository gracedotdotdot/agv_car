...
{
    ROS_INFO_STREAM("!!!!!!!!!! Yi Shin !!!!!!!!!!!!!!!!");
    
    float v = vel_cmd.linear.x * 0.75;
    float omega = vel_cmd.angular.z * 0.25;

    ROS_INFO_STREAM("cmd_vel:[" << v << " " << vel_cmd.linear.y << " " << omega << "]");

    Data_holder::get()->velocity.v_liner_x = v*100;
    Data_holder::get()->velocity.v_liner_y = vel_cmd.linear.y*100;
    Data_holder::get()->velocity.v_angular_z = omega*100;
