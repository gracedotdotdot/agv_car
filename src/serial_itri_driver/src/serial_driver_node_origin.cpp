...
const double WheelDistance = 0.24;		//m //0.224
const double WheelRadius = 0.085;		//m
...
std_msgs::Float64MultiArray Odom_Euler_Angle;
...
int main (int argc, char** argv)
{
	...
	ros::NodeHandle nh;
	//
	ros::Publisher left_motor_info_pub = nh.advertise<std_msgs::UInt8MultiArray>("left_motor_info", 1000);
	ros::Publisher right_motor_info_pub = nh.advertise<std_msgs::UInt8MultiArray>("right_motor_info", 1000);
	//
	ros::Subscriber cmd_sub = nh.subscribe("cmd_vel", 1000, write_serial);
	ros::Publisher odom_pub = nh.advertise<nav_msgs::Odometry>("odom", 1000);
	ros::Publisher Odom_Euler_Angle_Pub = nh.advertise<std_msgs::Float64MultiArray>("odom_euler_angle", 100);
	...
	while(ros::ok()){
		...
		//left_motor_info_pub.publish(array);
		ros::Duration(0.01).sleep();
		serR.write(array.data);	
		//right_motor_info_pub.publish(array);	
		ros::Duration(0.01).sleep();
		Lsave[0] = 0;
		Lsave[1] = 0;	
		Rsave[0] = 0;
		...
		ROS_INFO("Encoder %d %d",(int)encoderPulse[0],(int)encoderPulse[1]);
		//encoderPulse[0] += 1;
		//encoderPulse[1] += 1;
		//
		...
		//cout << "OdometryResult : \n";
		// 0.95
		WV[0] = ((double)d_encoderPulse[0] / EncoderPlus) * 0.95 *(2.0 * M_PI * WheelRadius) / GearRatio / DriverSampleTime;
		WV[1] = ((double)d_encoderPulse[1] / EncoderPlus) * 0.95 *(2.0 * M_PI * WheelRadius) / GearRatio / DriverSampleTime;
		...
		Odom_Euler_Angle.data.clear();
		Odom_Euler_Angle.data.push_back(0);
		Odom_Euler_Angle.data.push_back(0);
		Odom_Euler_Angle.data.push_back(Odometry[2]);
		...
		//set the position(Convert Direct)
		...
		Odom_Euler_Angle_Pub.publish(Odom_Euler_Angle);
		loop_rate.sleep();
		ros::spinOnce();
	}
	serL.close();
...
{
	//Direct Switch
	double Direct_Switch_Speed_linear_X = -1*(speed->linear.x);
	//m/s
	...
	int rightmotorvalue = (int)(round(vr / (2.0 * M_PI *  (WheelRadius + 0.0013) ) * 60.0));
	//Max rpm
