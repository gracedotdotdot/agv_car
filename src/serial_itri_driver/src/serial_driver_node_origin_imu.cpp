...
const double WheelDistance = 0.22;//m //0.2353, 0.224
...
int main (int argc, char** argv)
{
	...
	ros::Publisher odom_pub = nh.advertise<nav_msgs::Odometry>("odom_origin", 1000);
	...
	while(ros::ok()){
		...
		//double Conversation[36] = {0.0025,0,0,0,0,0};
		//Conversation
		...
		odom.header.frame_id = "odom_origin";
		...
		if(LinearVelocity == 0)
		{
			...
			//memcpy(odom.pose.covariance,ODOM_POSE_COVARIANCE2,sizeof(double)*36);
			//memcpy(odom.twist.covariance,ODOM_TWIST_COVARIANCE2,sizeof(double)*36);
