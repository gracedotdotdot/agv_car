...
int main(int argc, char **argv)
{
	...
	{
		int Mode = Get_Mode();
		//ROS_INFO("%d %d %d %d",UltraSonic_Pro.UltraSonic_Mean[0],UltraSonic_Pro.UltraSonic_Mean[1],UltraSonic_Pro.UltraSonic_Mean[2],UltraSonic_Pro.UltraSonic_Mean[3]);
		/*
		for(int i=0; i<4; i++)
		{
			cout << UltraSonic_Pro.UltraSonic_Mean[i] << ",";
		}
		cout << endl;
		*/
		if(Mode==2)
		{
			Remote_time = 0;
			//_AOA_Box.Check_AOA_Status();
			//cout << _AOA_Box.Calc_Std() << "," << (int)AOA_Val[0] << endl;
			//cout << _AOA_Box.AOA_Status << endl;
			/*
			if(_AOA_Box.Calc_Std()>10)
			{
				AOA_Val[0] = AOA_Pre_Angle;
			}
			else
			{
				AOA_Pre_Angle = AOA_Val[0];
			}
			*/
			/*
			if(Std!=-1)
			{
				ROS_INFO("Std_Diff=%f,Instance_Angle=%d",Std,(int)AOA_Val[0]);
			}
			*/
			//_AOA_Box.Calc_Std();
			//ROS_INFO("Std=%f",_AOA_Box.Calc_Std());
			cmd.linear.x = MCL.v;
			cmd.angular.z = MCL.w;
