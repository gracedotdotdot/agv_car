...
class Move_Control
{
	...
	int Ultra_Arib()
	{
		...
		//Slow down
		if((UltraBox_Mean[1] <= Short_Dist && UltraBox_Mean[1]>2) || (UltraBox_Mean[0] <= Short_Dist && UltraBox_Mean[0]>2) || (UltraBox_Mean[2] <= Short_Dist && UltraBox_Mean[2]>2))
		{
			//cout << UltraBox_Mean[0] << "," << UltraBox_Mean[1] << "," << UltraBox_Mean[2] << endl;
			this->v -= De_in_crease_Speed;
			this->w = 0;
			if(this->v <= 0.01)
			{
    ...
    void Control()
    {
	    if((UltraBox_Mean[0]*UltraBox_Mean[1]*UltraBox_Mean[2])>0)
	    {
			...
			//Error distance
			double Error_Distance = this->w_now - this->w_pre;
			if(Error_Distance > 0)
			{
				this->w += 0.001;
			}
