...
const double EncoderPlus = 2000.0*2.0; //4 pole
...
int Right_E_Dist_Now, Left_E_Dist_Now;
int Right_E_Dist_pre, Left_E_Dist_Pre;
int Right_CE_Dist_Now, Left_CE_Dist_Now;
int Right_Dist_Pre, Left_Dist_Pre;
int RX_Car_Buff[4];
bool Push_Flag = 0;
...
{
	//Initial Variable
	unsigned char LRXbuff = 0;
	unsigned char RRXbuff = 0;
	//Save Encode Value
	unsigned char Temp_Encode[3] = {0};
	//Send Read Cmd
	cmd.data.clear();
	...
	*/
	...
	while(serR.available()>=1)
	{
		...
		if(RRXbuff==0xE4)
		{
			serR.read(Temp_Encode,1);
			serR.read(Temp_Encode+1,1);
			serR.read(Temp_Encode+2,1);
			//CheckSum
			if((int)Temp_Encode[2] == ((int)(0xE4+Temp_Encode[0]+Temp_Encode[1])&0xFF))
			{
				SerR_use=1;
				break;
			}
	...
	if(SerR_use)
	{
		RX_Car_Buff[0] = (int)(Temp_Encode[0] << 8 | Temp_Encode[1]);
		/*
		if(((int)(Temp_Encode[0] << 8 | Temp_Encode[1]))!=0)
		{
			RX_Car_Buff[0] = 65535 - (int)(Temp_Encode[0] << 8 | Temp_Encode[1]);
		}
		else
		{
			RX_Car_Buff[0] = (int)(Temp_Encode[0] << 8 | Temp_Encode[1]);
		}
		if(RX_Car_Buff[0]>=255)
		{
			RX_Car_Buff[0] = -(65535 - RX_Car_Buff[0]); 
		}*/
	}
	/********************************************************************/
	unsigned char Temp_Encode_L[3] = {0};
	bool SerL_use = 0;
	while(serL.available()>=1)
	{
		serL.read(&LRXbuff,1);
		if(LRXbuff==0xE4)
		{
			serL.read(Temp_Encode_L,1);
			serL.read(Temp_Encode_L+1,1);
			serL.read(Temp_Encode_L+2,1);
			if((int)Temp_Encode_L[2] == ((int)(0xE4+Temp_Encode_L[0]+Temp_Encode_L[1])&0xFF))
			{
				SerL_use=1;
				break;
			}
	...
	if(SerL_use)
	{
		RX_Car_Buff[2] = (int)(Temp_Encode_L[0] << 8 | Temp_Encode_L[1]);
		//if(RX_Car_Buff[2]>=255)
		//{
		//	RX_Car_Buff[2] = -(65535 - RX_Car_Buff[2]); 
		//}

		/*
		if(((int)(Temp_Encode_L[0] << 8 | Temp_Encode_L[1]))!=0)
		{
			RX_Car_Buff[2] = 65535 - (int)(Temp_Encode_L[0] << 8 | Temp_Encode_L[1]);
		}
		else
		{
			RX_Car_Buff[2] = (int)(Temp_Encode_L[0] << 8 | Temp_Encode_L[1]);
	...
	//cout << (int)Temp_Encode[0] << "," << (int)Temp_Encode[1] << "," << (int)Temp_Encode[2] << ","<< (int)((0xE4+Temp_Encode[0]+Temp_Encode[1])&0xFF) << endl;
	/**************************************************/
	/**************************************************
	//Get Wheel Speed
	for(int i=0;i<32;i++)
	{
		if (RRXbuff[i]==0xE4)
		{
			RX_Car_Buff[0] = (int)(RRXbuff[i+1] << 8 | RRXbuff[i+2]);
			break;
		}
	...
	for(int i=0;i<32;i++)
	{
		if (LRXbuff[i]==0xE4)
		{
			RX_Car_Buff[2] = (int)(LRXbuff[i+1] << 8 | LRXbuff[i+2]);
			RX_Car_Buff[2] = 65535 - (int)RX_Car_Buff[2];	
			break;
		}
	}
	if(RX_Car_Buff[0] >= 65535)RX_Car_Buff[0]=0;
	if(RX_Car_Buff[2] >= 65535)RX_Car_Buff[2]=0;
	//cout << (int)RX_Car_Buff[0] << ","; 
	//cout << (int)RX_Car_Buff[2] << endl;
	*****************************************************************/
	//cout << "ER:" <<(int)RX_Car_Buff[0] << ","; 
	//cout << "EL:" << (int)RX_Car_Buff[2] << endl;
	return 1;
}
...
int main (int argc, char** argv)
{
	...
	while(ros::ok()){
		...
		ROS_INFOROS_INFO_STREAM("odom x: " << Odometry[0] << " odom y: " << Odometry[1] << " odom theta: " << Odometry[2] * 180.0 / M_PI);
