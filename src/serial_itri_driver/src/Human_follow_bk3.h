...
#define Limit_Angle 0.15
#define Limit_Angle_Work 70
//Speed Obj
...
class Move_Control
{
    ...
    void Control()
    {
	    if((UltraBox_Mean[0]*UltraBox_Mean[1]*UltraBox_Mean[2])>0)
	    {
			...
			/************************************************************************************/
			//Error distance
			Error_Distance = this->w_now - this->w_pre;
			if(Error_Distance > 0)
			{
				this->w += 0.001;
			}
